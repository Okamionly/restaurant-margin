...
// ============ RECIPES — moved to api-lib/routes/recipes.ts ============
// Mounted at line ~1026: app.use('/api/recipes', authWithRestaurant, recipesRoutes)
// 10 routes : list, detail, create, update, clone, delete, restore, photo add/remove, share
...
app.post('/api/invoices/scan', authWithRestaurant, async (req, res) => {
  try {
    ...
    if (!data || !mimeType) return res.status(400).json({ error: 'fichier (base64) et mimeType requis' });
    ...
    if (isWordExcel) return res.status(415).json({ error: "Word et Excel ne sont pas encore lus directement. Exportez la facture en PDF (Fichier > Enregistrer sous > PDF) puis rescannez." });
...
- Reponds avec un objet JSON valide uniquement.`;
    ...
    if (isImage) {
      ...
      return res.status(415).json({ error: 'Format non supporte. Acceptes : image (JPG/PNG/WebP), PDF, Markdown/texte/CSV. Pour Word/Excel, convertissez en PDF.' });
    }
    ...
    try {
      ...
      if (truncatedByModel) {
        ...
        if (itemsStart !== -1 && lastComplete > itemsStart) {
          try {
            const repaired = JSON.parse(jsonStr.slice(0, lastComplete + 1) + '], "totalHT": null, "totalTTC": null, "tva": null, "truncated": true}');
            return res.json(repaired);
          } catch { /* irrecuperable, message clair ci-dessous */ }
        }
        return res.status(422).json({ error: 'Document trop long pour un seul scan (resultat tronque). Scannez le document page par page, ou envoyez uniquement la page des totaux et lignes produits.' });
      }
