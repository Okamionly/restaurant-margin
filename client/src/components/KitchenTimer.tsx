    ...
    <>
        ...
        <div className="fixed bottom-36 right-4 sm:right-6 z-40 w-[calc(100vw-2rem)] sm:w-80 max-h-[70vh] flex flex-col bg-[#FAFAFA] dark:bg-[#0A0A0A] rounded-2xl shadow-2xl border border-[#E5E7EB] dark:border-[#1A1A1A] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700 bg-slate-800/50">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-white text-sm">Kitchen Timer</span>
              {timers.length > 0 && (
                <span className="text-xs text-slate-400">({timers.length})</span>
              )}
            </div>
            <button
          ...
          <div className="px-4 py-3 border-b border-slate-700/50 space-y-2">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-teal-500"
            ...
            <div className="flex gap-2 items-center">
              <div className="flex items-center gap-1.5 flex-1">
                <input
                  ...
                  className="w-20 bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-white text-center focus:outline-none focus:border-teal-500"
                  onKeyDown={e => e.key === 'Enter' && addTimer()}
                />
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs text-slate-400 truncate max-w-[120px]">{t.label}</span>
                    <button
      ...
      >
        ...
        {activeCount > 0 && !isOpen && (
          <span className="absolute -top-1 -right-1 w-5 h-5 bg-teal-400 text-slate-900 text-xs font-bold rounded-full flex items-center justify-center">
            {activeCount}
          </span>
        )}
