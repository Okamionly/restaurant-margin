...
export default function TrialPaywallGuard({ feature, children, queryLimit, currentQueries }: TrialPaywallGuardProps) {
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-white dark:bg-[#0A0A0A] border border-[#E5E7EB] dark:border-[#1A1A1A] rounded-2xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-[#F5F5F5] dark:from-[#171717] to-white dark:to-[#0A0A0A] border-b border-[#E5E7EB] dark:border-[#1A1A1A] px-6 py-5">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div className="rounded-xl border border-[#E5E7EB] dark:border-[#1A1A1A] p-4 text-center">
              <p className="text-sm font-bold text-[#111111] dark:text-white">Pro</p>
              <p className="text-2xl font-bold text-teal-500 mt-1">29<span className="text-sm font-normal text-[#737373]">/mois</span></p>
              <p className="text-xs text-[#737373] mt-1">1 restaurant</p>
            </div>
            <div className="rounded-xl border-2 border-teal-500/40 bg-teal-50/50 dark:bg-teal-900/10 p-4 text-center relative">
              ...
              <p className="text-sm font-bold text-[#111111] dark:text-white mt-1">Business</p>
              <p className="text-2xl font-bold text-teal-500 mt-1">79<span className="text-sm font-normal text-teal-400/70">/mois</span></p>
              <p className="text-xs text-[#737373] mt-1">Multi-sites</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-1.5">
              {PRO_FEATURES.map((f) => (
                <div key={f} className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-[#525252] dark:text-[#A3A3A3]">{f}</span>
                </div>
              ))}
            </div>
          </div>
