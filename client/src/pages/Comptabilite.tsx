...
function generateMonthlyData(): MonthlyData[] {
  const months = [
    { key: '2025-04', label: 'Avr 2025' },
    { key: '2025-05', label: 'Mai 2025' },
    { key: '2025-06', label: 'Juin 2025' },
    { key: '2025-07', label: 'Juil 2025' },
    { key: '2025-08', label: 'Août 2025' },
    { key: '2025-09', label: 'Sep 2025' },
    { key: '2025-10', label: 'Oct 2025' },
    { key: '2025-11', label: 'Nov 2025' },
    { key: '2025-12', label: 'Déc 2025' },
    { key: '2026-01', label: 'Jan 2026' },
    { key: '2026-02', label: 'Fév 2026' },
    { key: '2026-03', label: 'Mar 2026' },
  ];

  // Seasonal variation: summer higher, winter dip, December spike
  const seasonalFactor: Record<string, number> = {
    '01': 0.82, '02': 0.85, '03': 0.95, '04': 0.92,
    '05': 1.0, '06': 1.12, '07': 1.18, '08': 1.05,
    '09': 1.08, '10': 1.02, '11': 0.88, '12': 1.15,
  };

  return months.map(({ key, label }) => {
    const mm = key.split('-')[1];
    const factor = seasonalFactor[mm] || 1;
    const baseCA = 35000;
    const ca = Math.round(baseCA * factor + (Math.random() - 0.5) * 4000);
    const matiereRate = 0.28 + Math.random() * 0.06;
    const personnelRate = 0.30 + Math.random() * 0.06;
    const loyerFixed = 3200;
    const energieBase = 1200 + Math.random() * 600;
    const diversBase = 800 + Math.random() * 500;

    const matiere = Math.round(ca * matiereRate);
    const personnel = Math.round(ca * personnelRate);
    const energie = Math.round(energieBase);
    const divers = Math.round(diversBase);

    // TVA split: 60% at 10% (sur place), 25% at 20% (alcool), 15% at 5.5% (emporter)
    const tva55 = Math.round(ca * 0.15);
    const tva10 = Math.round(ca * 0.60);
    const tva20 = Math.round(ca * 0.25);

    return {
      mois: key,
      moisLabel: label,
      ca,
      charges: matiere + personnel + loyerFixed + energie + divers,
      matiere,
      personnel,
      loyer: loyerFixed,
      energie,
...
function generateSalesData(monthlyData: MonthlyData[]): SaleEntry[] {
  const entries: SaleEntry[] = [];
  const clients = ['', 'Entreprise Dupont', 'SCI Martin', 'Groupe Leclerc', 'Particulier', ''];
  const descriptions = [
    'Service midi', 'Service soir', 'Traiteur événement', 'Vente à emporter',
    'Service brunch', 'Banquet privé', 'Menu du jour', 'Carte complète',
  ];
  const paiements: PaymentMode[] = ['CB', 'Espèces', 'CB', 'CB', 'Ticket resto', 'Chèque', 'Virement', 'CB'];
  let id = 1;

  monthlyData.forEach((m) => {
    const [year, month] = m.mois.split('-');
    const daysInMonth = new Date(+year, +month, 0).getDate();
    const nbEntries = 20 + Math.floor(Math.random() * 10);
    const avgHT = m.ca / nbEntries;

    for (let i = 0; i < nbEntries; i++) {
      const day = String(1 + Math.floor(Math.random() * daysInMonth)).padStart(2, '0');
      const montantHT = Math.round(avgHT * (0.4 + Math.random() * 1.2));
      const tvaRate = [0.055, 0.10, 0.10, 0.10, 0.20][Math.floor(Math.random() * 5)];
      const tva = Math.round(montantHT * tvaRate);

      entries.push({
        id: id++,
        date: `${year}-${month}-${day}`,
        invoiceNum: `FC-${year}${month}-${String(id).padStart(4, '0')}`,
        client: clients[Math.floor(Math.random() * clients.length)],
        description: descriptions[Math.floor(Math.random() * descriptions.length)],
        montantHT,
        tva,
        ttc: montantHT + tva,
        paiement: paiements[Math.floor(Math.random() * paiements.length)],
      });
    }
  });
...
function generateExpensesData(monthlyData: MonthlyData[]): ExpenseEntry[] {
  const entries: ExpenseEntry[] = [];
  const categories = [
    { cat: 'Matières premières', fournisseurs: ['Pomona', 'Rungis Express', 'Metro', 'Brake France', 'Transgourmet'] },
    { cat: 'Personnel', fournisseurs: ['Salaires', 'Charges sociales', 'Intérim RestauStaff'] },
    { cat: 'Loyer', fournisseurs: ['SCI Immobilière'] },
    { cat: 'Énergie', fournisseurs: ['EDF Pro', 'Engie', 'TotalEnergies'] },
    { cat: 'Entretien', fournisseurs: ['CleanPro', 'Hygiène Plus'] },
    { cat: 'Assurance', fournisseurs: ['AXA Pro', 'Allianz'] },
    { cat: 'Marketing', fournisseurs: ['Google Ads', 'Meta Business', 'Imprimerie Locale'] },
    { cat: 'Divers', fournisseurs: ['Amazon Pro', 'Lyreco', 'Manutan'] },
  ];
  let id = 1;
  ...
  monthlyData.forEach((m) => {
    ...
    const amounts: Record<string, number> = {
      'Matières premières': m.matiere,
      'Personnel': m.personnel,
      'Loyer': m.loyer,
      'Énergie': m.energie,
      'Entretien': Math.round(300 + Math.random() * 200),
      'Assurance': 450,
      'Marketing': Math.round(200 + Math.random() * 300),
      'Divers': Math.round(m.divers * 0.3),
    };

    categories.forEach(({ cat, fournisseurs }) => {
      const total = amounts[cat] || 500;
      const nbEntries = cat === 'Loyer' || cat === 'Assurance' ? 1 : 2 + Math.floor(Math.random() * 3);
      const perEntry = Math.round(total / nbEntries);

      for (let i = 0; i < nbEntries; i++) {
        const day = String(1 + Math.floor(Math.random() * 28)).padStart(2, '0');
        const ht = perEntry + Math.round((Math.random() - 0.5) * perEntry * 0.2);
        const tvaRate = cat === 'Loyer' ? 0 : 0.20;
        const tva = Math.round(ht * tvaRate);

        entries.push({
          id: id++,
          date: `${year}-${month}-${day}`,
          fournisseur: fournisseurs[Math.floor(Math.random() * fournisseurs.length)],
          categorie: cat,
          montantHT: ht,
          tva,
          ttc: ht + tva,
          description: `${cat} - ${m.moisLabel}`,
        });
      }
    });
  });
