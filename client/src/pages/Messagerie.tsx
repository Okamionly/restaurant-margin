    ...
    <div className="flex flex-col h-[calc(100vh-12rem)]">
      ...
      <div className="flex flex-1 min-h-0 bg-slate-900/50 rounded-2xl border border-slate-800 overflow-hidden">
        ...
        <div className={`w-full md:w-[380px] border-r border-slate-800 flex flex-col ${mobileShowMail ? 'hidden md:flex' : 'flex'}`}>
          ...
          {/* Inbox label */}
          <div className="px-4 py-2 flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-800/50">
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-xs mt-0.5 truncate ${conv.unread > 0 ? 'text-slate-200' : 'text-slate-400'}`}>
        ...
        <div className={`flex-1 flex flex-col ${!mobileShowMail ? 'hidden md:flex' : 'flex'}`}>
          {activeConv ? (
              ...
              <div className="px-6 py-4 border-b border-slate-800 bg-slate-900/60">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-slate-400 ml-2 text-xs">&lt;{activeConv.email || activeConv.members[0] || 'inconnu'}&gt;</span>
                  </div>
                </div>
              </div>
              ...
              {replyOpen ? (
                <div className="border-t border-slate-800 bg-slate-900/80 p-4">
                  <div className="flex items-center gap-2 mb-2 text-xs text-slate-400">
                    ...
                    <button onClick={() => setReplyOpen(false)} className="ml-auto text-slate-400 hover:text-slate-300">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-3 rounded-lg bg-slate-800 border border-slate-700 text-sm text-slate-200 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-500 resize-none"
                    autoFocus
                  />
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-300">
              ...
              <p className="text-lg font-medium text-slate-400">Sélectionnez un message</p>
              <p className="text-sm mt-1 text-slate-300">Choisissez un email pour le lire</p>
            </div>
          )}
        </div>
      </div>
          ...
          >
            ...
            <div className="px-4 py-2 space-y-0">
              <div className="flex items-center border-b border-slate-800 py-2">
                <span className="text-xs text-slate-400 w-10">À :</span>
                <input
                  type="email"
                  value={composeTo}
                  onChange={(e) => setComposeTo(e.target.value)}
                  placeholder="email@exemple.com"
                  className="flex-1 bg-transparent text-sm text-slate-200 border-0 focus:ring-0 placeholder:text-slate-300 p-0"
                />
              </div>
              <div className="flex items-center border-b border-slate-800 py-2">
                <span className="text-xs text-slate-400 w-10">Objet :</span>
                <input
                  type="text"
            ...
            <div className="flex-1 px-4 py-2">
              <textarea
                ...
                rows={8}
                className="w-full bg-transparent text-sm text-slate-200 border-0 focus:ring-0 placeholder:text-slate-300 resize-none"
              />
            </div>
