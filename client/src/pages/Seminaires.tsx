...
const STATUS_COLORS: Record<EventStatus, { bg: string; text: string; border: string; dot: string }> = {
  'Demande':       { bg: 'bg-mono-950 dark:bg-[#171717]/50', text: 'text-[#9CA3AF] dark:text-mono-500', border: 'border-mono-900 dark:border-mono-200', dot: 'bg-mono-900 dark:bg-mono-200' },
...
const TYPE_COLORS: Record<EventType, string> = {
  ...
  'Autre':        'bg-mono-950 dark:bg-[#171717] text-[#9CA3AF] dark:text-mono-500 dark:bg-[#171717]/40 dark:text-mono-700',
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-mono-100 dark:text-white dark:text-white flex items-center gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-mono-950 dark:bg-[#171717] rounded-lg p-0.5">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                viewMode === 'calendar' ? 'bg-white dark:bg-mono-50 text-mono-100 dark:text-white shadow-sm' : 'text-[#9CA3AF] dark:text-mono-500'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                viewMode === 'analytics' ? 'bg-white dark:bg-mono-50 text-mono-100 dark:text-white shadow-sm' : 'text-[#9CA3AF] dark:text-mono-500'
              }`}
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-mono-100 dark:bg-white hover:bg-[#333] dark:hover:bg-[#E5E5E5] text-white rounded-lg text-sm font-semibold transition-colors shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-mono-50 rounded-xl border border-mono-900 dark:border-mono-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">Événements ce mois</p>
              <p className="text-2xl font-bold text-mono-100 dark:text-white dark:text-white">{thisMonthEvents.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-mono-50 rounded-xl border border-mono-900 dark:border-mono-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">CA prévisionnel</p>
              <p className="text-2xl font-bold text-mono-100 dark:text-white dark:text-white">{formatEuro(caPrevi)}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-mono-50 rounded-xl border border-mono-900 dark:border-mono-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">Taux de conversion</p>
              <p className="text-2xl font-bold text-mono-100 dark:text-white dark:text-white">{tauxConversion}%</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-mono-50 rounded-xl border border-mono-900 dark:border-mono-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">Prochain événement</p>
              {prochainEvent ? (
                <p className="text-sm font-bold text-mono-100 dark:text-white dark:text-white truncate">
                  {formatDateShort(prochainEvent.date)} — {prochainEvent.clientNom.split(' ')[0]}
                </p>
              ) : (
                <p className="text-sm text-[#9CA3AF] dark:text-mono-500">Aucun</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2 bg-white dark:bg-mono-50 border border-mono-900 dark:border-mono-200 rounded-lg text-sm text-mono-100 dark:text-white dark:text-white placeholder:text-[#6B7280] dark:text-mono-700 focus:outline-none focus:ring-2 focus:ring-mono-100 dark:ring-white"
        />
      </div>
          ...
          {STATUS_COLUMNS.map(status => {
            ...
            return (
              <div key={status} className="flex-shrink-0 w-72">
                ...
                {/* Cards */}
                <div className={`border-x border-b ${col.border} rounded-b-xl bg-mono-1000 dark:bg-mono-50 dark:bg-mono-1000/50 dark:bg-mono-50/50 p-2 space-y-2 min-h-[200px]`}>
                  {columnEvents.length === 0 && (
                    <div className="text-xs text-[#9CA3AF] dark:text-mono-500 text-center py-8">Aucun événement</div>
                  )}
                  {columnEvents.map(ev => (
                    <div
                      key={ev.id}
                      onClick={() => { setSelectedEvent(ev); setShowDetailModal(true); }}
                      className="bg-white dark:bg-mono-50 rounded-lg border border-mono-900 dark:border-mono-200 p-3 cursor-pointer hover:shadow-md hover:border-teal-300 dark:hover:border-teal-600 transition-all group"
                    >
                      {/* Type badge */}
                      ...
                      {/* Client */}
                      <h4 className="text-sm font-semibold text-mono-100 dark:text-white dark:text-white truncate">{ev.clientNom}</h4>
                      {ev.clientEntreprise && (
                        <p className="text-xs text-[#9CA3AF] dark:text-mono-500 flex items-center gap-1 mt-0.5">
                          <Building2 className="w-3 h-3" /> {ev.clientEntreprise}
                        </p>
                      )}
                      {/* Info row */}
                      <div className="flex items-center gap-3 mt-2 text-xs text-[#9CA3AF] dark:text-mono-500">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-sm font-bold text-mono-100 dark:text-white dark:text-white">{formatEuro(ev.totalEstime)}</span>
                        {/* Move arrows */}
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {STATUS_COLUMNS.indexOf(ev.status) > 0 && (
                            <button
                              onClick={e2 => { e2.stopPropagation(); moveEvent(ev.id, 'prev'); }}
                              className="p-1 rounded hover:bg-mono-950 dark:hover:bg-[#171717] dark:hover:bg-[#171717]"
                              title="Reculer"
                            >
      ...
      {/* ═══════════ CALENDAR VIEW ═══════════ */}
      {viewMode === 'calendar' && (
        <div className="bg-white dark:bg-mono-50 rounded-xl border border-mono-900 dark:border-mono-200 overflow-hidden">
          {/* Month nav */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-mono-900 dark:border-mono-200">
            <button onClick={prevMonth} className="p-2 rounded-lg hover:bg-mono-950 dark:hover:bg-[#171717] dark:hover:bg-[#171717] transition-colors">
              <ChevronLeft className="w-5 h-5 text-[#6B7280] dark:text-mono-700" />
            </button>
            <h2 className="text-lg font-bold text-mono-100 dark:text-white dark:text-white capitalize">{monthLabel}</h2>
            <button onClick={nextMonth} className="p-2 rounded-lg hover:bg-mono-950 dark:hover:bg-[#171717] dark:hover:bg-[#171717] transition-colors">
          ...
          <div className="grid grid-cols-7 border-b border-mono-900 dark:border-mono-200">
            {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map(d => (
              <div key={d} className="text-center text-xs font-semibold text-[#9CA3AF] dark:text-mono-500 py-2">{d}</div>
            ))}
          </div>
          {/* Calendar grid */}
          <div className="grid grid-cols-7">
            ...
            {Array.from({ length: firstDay }).map((_, i) => (
              <div key={`e${i}`} className="min-h-[100px] border-b border-r border-mono-900 dark:border-mono-200/50" />
            ))}
            {/* Day cells */}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  key={day}
                  className={`min-h-[100px] border-b border-r border-mono-900 dark:border-mono-200/50 p-1.5 ${
                    isToday ? 'bg-teal-50/50 dark:bg-teal-900/10' : ''
                  }`}
                >
                  <span className={`text-xs font-medium ${isToday ? 'bg-mono-100 dark:bg-white text-white px-1.5 py-0.5 rounded-full' : 'text-[#6B7280] dark:text-mono-700'}`}>
                    {day}
                  </span>
                  <div className="mt-1 space-y-1">
                    ...
                    {dayEvents.length > 3 && (
                      <span className="text-[10px] text-[#9CA3AF] dark:text-mono-500 pl-1">+{dayEvents.length - 3} autres</span>
                    )}
                  </div>
                </div>
              );
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white dark:bg-mono-50 rounded-2xl border border-mono-900 dark:border-mono-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">CA Total Confirme</p>
                  <p className="text-xl font-bold text-mono-100 dark:text-white">{formatEuro(analytics.totalRevenue)}</p>
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-mono-50 rounded-2xl border border-mono-900 dark:border-mono-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">Panier Moyen</p>
                  <p className="text-xl font-bold text-mono-100 dark:text-white">{formatEuro(analytics.avgPerEvent)}</p>
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-mono-50 rounded-2xl border border-mono-900 dark:border-mono-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">Convives Moyens</p>
                  <p className="text-xl font-bold text-mono-100 dark:text-white">{analytics.avgGuests} pers.</p>
                </div>
              </div>
            </div>
            <div className="bg-white dark:bg-mono-50 rounded-2xl border border-mono-900 dark:border-mono-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium">Evenements Soldes</p>
                  <p className="text-xl font-bold text-mono-100 dark:text-white">{analytics.completedEvents}</p>
                </div>
              </div>
            </div>
          </div>
                ...
                {Object.entries(analytics.byType).sort((a, b) => b[1] - a[1]).map(([type, count]) => {
                  ...
                  return (
                    <div key={type} className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-mono-100 dark:text-white w-12 text-right">{count}</span>
                      <span className="text-xs text-[#9CA3AF] dark:text-mono-500 w-10 text-right">{pct}%</span>
                    </div>
                  );
                })}
                {Object.keys(analytics.byType).length === 0 && (
                  <p className="text-sm text-[#9CA3AF] dark:text-mono-500 text-center py-4">Aucune donnee</p>
                )}
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-mono-900 dark:border-mono-200">
                    <th className="text-left py-2 text-xs font-semibold text-[#9CA3AF] dark:text-mono-500 uppercase tracking-wider">Mois</th>
                    <th className="text-center py-2 text-xs font-semibold text-[#9CA3AF] dark:text-mono-500 uppercase tracking-wider">Evenements</th>
                    <th className="text-right py-2 text-xs font-semibold text-[#9CA3AF] dark:text-mono-500 uppercase tracking-wider">CA Confirme</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-mono-950 dark:divide-mono-200/50">
                    ...
                    .map(([month, data]) => {
                      ...
                      return (
                        <tr key={month} className="hover:bg-[#F9FAFB] dark:hover:bg-[#171717]/30">
                          <td className="py-2.5 font-medium text-mono-100 dark:text-white capitalize">{label}</td>
                          <td className="py-2.5 text-center">
                            <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-mono-950 dark:bg-[#171717] text-xs font-bold text-mono-100 dark:text-white">
                              {data.count}
                            </span>
                          </td>
                          <td className="py-2.5">
                            <div className="flex items-center justify-end gap-2">
                              <div className="w-24 h-2 bg-mono-950 dark:bg-[#171717] rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${barWidth}%` }} />
                              </div>
                              <span className="font-bold text-mono-100 dark:text-white text-right w-20">{formatEuro(data.revenue)}</span>
                            </div>
                          </td>
                        </tr>
                      );
                  ...
                  {Object.keys(analytics.byMonth).length === 0 && (
                    <tr>
                      <td colSpan={3} className="text-center py-8 text-[#9CA3AF] dark:text-mono-500">Aucun historique</td>
                    </tr>
                  )}
                </tbody>
              </table>
                ...
                .map(ev => {
                  ...
                  return (
                    <div
                      key={ev.id}
                      onClick={() => { setSelectedEvent(ev); setShowDetailModal(true); }}
                      className="flex items-center gap-4 p-3 rounded-xl hover:bg-[#F9FAFB] dark:hover:bg-[#171717]/30 cursor-pointer transition-colors border border-transparent hover:border-mono-900 dark:hover:border-mono-200"
                    >
                      <div className="text-center flex-shrink-0 w-12">
                        <div className="text-lg font-bold text-mono-100 dark:text-white leading-none">{new Date(ev.date + 'T00:00:00').getDate()}</div>
                        <div className="text-[10px] text-[#9CA3AF] dark:text-mono-500 uppercase">{new Date(ev.date + 'T00:00:00').toLocaleDateString('fr-FR', { month: 'short' })}</div>
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-semibold text-mono-100 dark:text-white truncate">{ev.clientNom}</h4>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${TYPE_COLORS[ev.type]}`}>{ev.type}</span>
                          <span className="text-xs text-[#9CA3AF] dark:text-mono-500">{ev.nbConvivesMax} convives</span>
                        </div>
                      </div>
                      ...
                      <span className="text-sm font-bold text-mono-100 dark:text-white flex-shrink-0 w-20 text-right">{formatEuro(ev.totalEstime)}</span>
                    </div>
                  );
                })}
              {events.length === 0 && (
                <p className="text-sm text-[#9CA3AF] dark:text-mono-500 text-center py-8">Aucun evenement</p>
              )}
            </div>
          </div>
        </div>
      ...
      <Modal isOpen={showNewModal} onClose={() => setShowNewModal(false)} title="Nouvel événement" className="max-w-3xl">
        <div className="space-y-6">
          {/* Client info */}
          <div>
            <h4 className="text-sm font-bold text-[#9CA3AF] dark:text-mono-500 mb-3 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Entreprise</label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Email</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Téléphone</label>
                <input
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Type *</label>
                <select
                  value={form.type}
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Date *</label>
                <input
                  type="date"
                  value={form.date}
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Heure début</label>
                <input
                  type="time"
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Heure fin</label>
                <input
                  type="time"
                  value={form.heureFin}
              ...
              <div className="sm:col-span-2">
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Salle / Espace</label>
                <select
                  value={form.salle}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Entrée</label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Plat</label>
                <input
                  value={form.menuPlat}
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Dessert</label>
                <input
                ...
                <label
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-colors ${
                    form.equipements.includes(eq.id)
                      ...
                      : 'bg-white dark:bg-black border-mono-900 dark:border-mono-200 text-[#6B7280] dark:text-mono-700 hover:border-mono-900 dark:border-mono-200 dark:hover:border-mono-200'
                  }`}
                >
                  <input
                    type="checkbox"
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Total estime</label>
                <div className="px-3 py-2 bg-mono-1000 dark:bg-mono-50/50 border border-mono-900 dark:border-mono-200 rounded-lg text-sm font-bold text-mono-100 dark:text-white">
                  {formatEuro(form.prixParPersonne * form.nbConvivesMax)}
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Arrhes ({form.arrhesPercent}%)</label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-20 px-3 py-2 bg-white dark:bg-black border border-mono-900 dark:border-mono-200 rounded-lg text-sm text-mono-100 dark:text-white dark:text-white focus:outline-none focus:ring-2 focus:ring-mono-100 dark:ring-white"
                  />
                  <span className="text-sm text-[#9CA3AF] dark:text-mono-500">%</span>
                  <span className="text-sm font-medium text-[#9CA3AF] dark:text-mono-500">
          ...
          {/* Notes */}
          <div>
            <label className="block text-xs font-medium text-[#6B7280] dark:text-mono-700 mb-1">Notes internes</label>
            <textarea
              value={form.notes}
              onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
              rows={3}
              placeholder="Informations complémentaires, allergies, demandes spéciales..."
              className="w-full px-3 py-2 bg-white dark:bg-black border border-mono-900 dark:border-mono-200 rounded-lg text-sm text-mono-100 dark:text-white dark:text-white placeholder:text-[#6B7280] dark:text-mono-700 focus:outline-none focus:ring-2 focus:ring-mono-100 dark:ring-white resize-none"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-mono-900 dark:border-mono-200">
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-[#6B7280] dark:text-mono-700 hover:text-mono-100 dark:hover:text-white dark:hover:text-mono-100 dark:hover:text-white transition-colors"
            >
              Annuler
            </button>
            <button
              onClick={handleCreate}
              className="px-6 py-2 bg-mono-100 dark:bg-white hover:bg-[#333] dark:hover:bg-[#E5E5E5] text-white rounded-lg text-sm font-semibold transition-colors"
            >
              Créer l'événement
            </button>
          </div>
          ...
          {(() => {
            ...
            return (
              <div className="space-y-6">
                ...
                {/* Client info */}
                <div className="bg-mono-1000 dark:bg-mono-50 dark:bg-mono-1000/50 dark:bg-mono-50/50 rounded-lg p-4 space-y-2">
                  <h4 className="text-sm font-bold text-[#9CA3AF] dark:text-mono-500">Client</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center gap-2 text-[#6B7280] dark:text-mono-700">
                      <Users className="w-4 h-4 text-[#9CA3AF] dark:text-mono-500" /> {ev.clientNom}
                    </div>
                    {ev.clientEntreprise && (
                      <div className="flex items-center gap-2 text-[#6B7280] dark:text-mono-700">
                        <Building2 className="w-4 h-4 text-[#9CA3AF] dark:text-mono-500" /> {ev.clientEntreprise}
                      </div>
                    )}
                    <div className="flex items-center gap-2 text-[#6B7280] dark:text-mono-700">
                      <Mail className="w-4 h-4 text-[#9CA3AF] dark:text-mono-500" /> {ev.clientEmail}
                    </div>
                    <div className="flex items-center gap-2 text-[#6B7280] dark:text-mono-700">
                      <Phone className="w-4 h-4 text-[#9CA3AF] dark:text-mono-500" /> {ev.clientTelephone}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="text-center p-3 bg-mono-1000 dark:bg-mono-50 dark:bg-mono-1000/50 dark:bg-mono-50/50 rounded-lg">
                    <CalendarDays className="w-5 h-5 text-teal-500 mx-auto mb-1" />
                    <p className="text-xs text-[#9CA3AF] dark:text-mono-500">Date</p>
                    <p className="text-sm font-bold text-mono-100 dark:text-white dark:text-white">{formatDate(ev.date)}</p>
                  </div>
                  <div className="text-center p-3 bg-mono-1000 dark:bg-mono-50 dark:bg-mono-1000/50 dark:bg-mono-50/50 rounded-lg">
                    <Clock className="w-5 h-5 text-amber-500 mx-auto mb-1" />
                    <p className="text-xs text-[#9CA3AF] dark:text-mono-500">Horaire</p>
                    <p className="text-sm font-bold text-mono-100 dark:text-white dark:text-white">{ev.heureDebut} - {ev.heureFin}</p>
                  </div>
                  <div className="text-center p-3 bg-mono-1000 dark:bg-mono-50 dark:bg-mono-1000/50 dark:bg-mono-50/50 rounded-lg">
                    <Users className="w-5 h-5 text-emerald-500 mx-auto mb-1" />
                    <p className="text-xs text-[#9CA3AF] dark:text-mono-500">Convives</p>
                    <p className="text-sm font-bold text-mono-100 dark:text-white dark:text-white">{ev.nbConvivesMin} - {ev.nbConvivesMax}</p>
                  </div>
                  <div className="text-center p-3 bg-mono-1000 dark:bg-mono-50 dark:bg-mono-1000/50 dark:bg-mono-50/50 rounded-lg">
                    <MapPin className="w-5 h-5 text-purple-500 mx-auto mb-1" />
                    <p className="text-xs text-[#9CA3AF] dark:text-mono-500">Salle</p>
                    <p className="text-sm font-bold text-mono-100 dark:text-white dark:text-white">{ev.salle}</p>
                  </div>
                </div>

                {/* Menu */}
                <div>
                  <h4 className="text-sm font-bold text-[#9CA3AF] dark:text-mono-500 mb-2 flex items-center gap-2">
                    <Utensils className="w-4 h-4" /> Menu
                  </h4>
                  <div className="bg-mono-1000 dark:bg-mono-50 dark:bg-mono-1000/50 dark:bg-mono-50/50 rounded-lg p-4">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                      <div>
                        <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium mb-0.5">Entrée</p>
                        <p className="text-mono-100 dark:text-white dark:text-white">{ev.menu.entree || '—'}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium mb-0.5">Plat</p>
                        <p className="text-mono-100 dark:text-white dark:text-white">{ev.menu.plat || '—'}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#9CA3AF] dark:text-mono-500 font-medium mb-0.5">Dessert</p>
                        <p className="text-mono-100 dark:text-white dark:text-white">{ev.menu.dessert || '—'}</p>
                      </div>
                    </div>
                ...
                {ev.equipements.length > 0 && (
                  <div>
                    <h4 className="text-sm font-bold text-[#9CA3AF] dark:text-mono-500 mb-2">Équipements</h4>
                    <div className="flex flex-wrap gap-2">
                      {ev.equipements.map(eqId => {
                        const eq = EQUIPEMENTS.find(x => x.id === eqId);
                        if (!eq) return null;
                        return (
                          <span key={eqId} className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-mono-950 dark:bg-[#171717]/50 text-[#6B7280] dark:text-mono-700">
                ...
                {ev.notes && (
                  <div>
                    ...
                    <p className="text-sm text-[#6B7280] dark:text-mono-700 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
                      {ev.notes}
                    </p>
                  </div>
                )}
                ...
                <div>
                  <h4 className="text-sm font-bold text-[#9CA3AF] dark:text-mono-500 mb-3 flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-[#9CA3AF] dark:text-mono-500 ml-auto">
                      {getChecklist(ev.id).filter(c => c.checked).length}/{getChecklist(ev.id).length}
                    </span>
                  </h4>
                  <div className="bg-mono-1000 dark:bg-mono-50/50 rounded-xl p-3 space-y-1.5">
                    {/* Progress bar */}
                    <div className="w-full h-1.5 bg-mono-900 dark:bg-mono-200 rounded-full overflow-hidden mb-3">
                      <div
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-sm transition-colors ${
                          item.checked
                            ...
                            : 'text-[#6B7280] dark:text-mono-700 hover:bg-white dark:hover:bg-[#171717]'
                        }`}
                      >
                        {item.checked ? (
                          <CheckSquare className="w-4 h-4 text-emerald-500 flex-shrink-0" />
                        ) : (
                          <Square className="w-4 h-4 text-[#D1D5DB] dark:text-mono-400 flex-shrink-0" />
                        )}
                ...
                {/* Timeline */}
                <div>
                  <h4 className="text-sm font-bold text-[#9CA3AF] dark:text-mono-500 mb-3">Historique</h4>
                  <div className="space-y-0">
                    {ev.timeline.map((entry, i) => (
                      <div key={i} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          <div className="w-2.5 h-2.5 rounded-full bg-teal-500 mt-1.5 flex-shrink-0" />
                          {i < ev.timeline.length - 1 && <div className="w-px flex-1 bg-mono-950 dark:bg-[#171717] my-1" />}
                        </div>
                        <div className="pb-3">
                          <p className="text-xs text-[#9CA3AF] dark:text-mono-500">{formatDate(entry.date)}</p>
                          <p className="text-sm text-[#9CA3AF] dark:text-mono-500">{entry.action}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* Action buttons */}
                <div className="flex flex-wrap gap-2 pt-3 border-t border-mono-900 dark:border-mono-200">
                  {ev.status === 'Demande' && (
                    <button
                      onClick={() => handleStatusAction(ev, 'envoyer_devis')}
                      className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-mono-100 dark:text-white rounded-lg text-sm font-semibold transition-colors"
                    >
                  ...
                  {(ev.status === 'Devis envoyé' || ev.status === 'Demande') && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-mono-100 dark:bg-white hover:bg-[#333] dark:hover:bg-[#E5E5E5] text-white rounded-lg text-sm font-semibold transition-colors"
                    >
